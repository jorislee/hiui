...
function mergeLocaleMessage(key, locales) {
	for (const locale in locales) {
		...
		if (!messages.menus) messages.menus = {};
...
class Hiui {
	...
	async rpc(method, param) {
		const {data} = await axios.post('/hiui-rpc', {method, param});
		return data;
	}
	...
	async init() {
		...
		if (locale === 'auto') i18n.global.locale = navigator.language;
	...
	install(app) {
		const hiui = {
			...
			setLocale: async (locale) => {
				if (locale !== 'auto' && !i18n.global.availableLocales.includes(locale)) return;

				await this.call('uci', 'set', {config: 'hiui', section: 'global', values: {locale}});
			...
			setTheme: async (theme) => {
				await this.call('uci', 'set', {config: 'hiui', section: 'global', values: {theme}});
				this.state.theme = theme;
			},
		...
		app.config.globalProperties.$hiui = hiui;
		app.config.globalProperties.$md5 = md5;
	}
}

export default new Hiui();
