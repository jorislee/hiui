...
export default {
		...
		return {
			columns: [
				...
				},
				{
					...
					key: 'rate',
					render: (r) => h('div', [h('p', this.wifiRate(r.rx_rate)), h('p', this.wifiRate(r.tx_rate))])
				}
			],
			stations: []
		};
	...
	methods: {
		wifiRate(rate) {
			...
			if (rate.ht || rate.vht) {
				if (rate.vht) s += ', VHT-MCS ' + rate.mcs;
				if (rate.nss) s += ', VHT-NSS ' + rate.nss;
				if (rate.ht) s += ', MCS ' + rate.mcs;
			}

			if (rate.he) {
				s += ', HE-MCS ' + rate.mcs;
				if (rate.nss) s += ', HE-NSS ' + rate.nss;
				if (rate.he_gi) s += ', HE-GI ' + rate.he_gi;
				if (rate.he_dcm) s += ', HE-DCM ' + rate.he_dcm;
			}

			return s;
		},
		getStations() {
			this.$hiui.call('wireless', 'stations').then(({stations}) => {
