...
export default {
	data() {
		return {
			loading: false,
			formValue: {
				hostname: this.$hiui.state.hostname,
				zonename: ''
			},
			rules: {
				hostname: {
					...
					trigger: 'blur',
					validator: (_, value) => {
						if (!value) return Error(this.$t('This field is required'));

						if (value.length <= 253 && (value.match(/^[a-zA-Z0-9_]+$/) || (value.match(/^[a-zA-Z0-9_][a-zA-Z0-9_\-.]*[a-zA-Z0-9]$/) && value.match(/[^0-9.]/)))) return;
	...
	methods: {
		handleSubmit() {
			this.$refs.form.validate(async (errors) => {
				...
				await this.$hiui.setHostname(this.formValue.hostname);
				...
				await this.$hiui.call('uci', 'set', {
					config: 'system',
					section: '@system[0]',
					values: {timezone, zonename}
				});

				await this.$hiui.reloadConfig('system');
