...
export default {
	...
	computed: {
		cpuUsage() {
			if (this.cpuTimes.length < 2) return {cpu: 0};
		...
		memInfo() {
			...
			if (memory.buffered) info.push(['Buffered', memory.buffered]);
			if (memory.cached) info.push(['Cached', memory.cached]);
	...
	methods: {
		...
		getCpuTimes() {
			this.$hiui.call('system', 'get_cpu_time').then(({times}) => {
				this.cpuTimes.push(times);
				if (this.cpuTimes.length === 3) this.cpuTimes.shift();
			});
		},
		getSysinfo() {
			this.$hiui.ubus('system', 'info').then((r) => {
				this.sysinfo = r;
			});
		},
		getWanNetworks() {
			this.$hiui.call('network', 'get_wan_networks').then(({networks}) => {
				this.wanNetworks = networks;
			});
		},
		getWan6Networks() {
			this.$hiui.call('network', 'get_wan6_networks').then(({networks}) => {
				this.wan6Networks = networks;
			});
		},
		renderNetworkInfo(net, ipv6) {
			const info = [['Protocol', net.proto]];

			if (ipv6) {
				const prefix = net['ipv6-prefix'][0];
				if (prefix) info.push(['Prefix Delegated', prefix.address + '/' + prefix.mask]);
	...
	created() {
		...
		this.$hiui.ubus('system', 'board').then((r) => {
			this.boardinfo = r;
		});
	}
};
