...
export default {
		...
		return {
			columns: [
				...
				},
				{
					...
					render: (r) =>
						h(resolveComponent('n-dynamic-tags'), {
							value: r.matchs,
	...
	computed: {
		...
		acls() {
			if (!this.allAcls || !this.group) return [];
	...
	methods: {
		handleSubmit() {
			this.loading = true;
			this.$hiui.call('acl', 'set', {acls: this.allAcls}).then(() => (this.loading = false));
		},
		addGroup() {
			let group = '';
			this.$dialog.create({
	...
	created() {
		this.$hiui.call('acl', 'load').then((acls) => {
			this.allAcls = acls;
